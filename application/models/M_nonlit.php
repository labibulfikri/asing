...
class M_nonlit extends CI_Model {
 ...
 public function __construct()
	...
	function verify ($tabel,$where){
     ...
     function get_data()
     {
          $sql = $this->db->query("SELECT * from t_nonlit 
                                        ORDER BY tgl_nonlit DESC")->result();
          return $sql;
     }

	 
	
	function get_dataCount($id){
		          $sql = $this->db->query("SELECT COUNT(t_resume.id_resume) as ini
											FROM t_resume
											join t_nonlit on t_nonlit.id_nonlit = t_resume.id_nonlit
											where t_nonlit.id_nonlit = $id ")->num_rows();										
     ...
     {
        $sql = $this->db->query("SELECT * from t_resume 
                                        join t_nonlit on t_nonlit.id_nonlit = t_resume.id_nonlit
                                        where t_nonlit.id_nonlit = $id 
                                        ORDER BY tgl_resume DESC")->result();
		return $sql;
     }

     function get_resumeRow($id)
     {
          $sql = $this->db->query("SELECT * from t_resume 
                                        join t_nonlit on t_nonlit.id_nonlit = t_resume.id_nonlit
                                        where t_resume.id_resume = $id
                                        ORDER BY tgl_resume DESC")->row();
          return $sql;
     }
	 
	 function getberkas_resume($id_resume)
     {
          $sql = $this->db->query("SELECT * from t_upload_resume 
                                        join t_resume on t_resume.id_resume = t_upload_resume.id_resume
                                        where t_upload_resume.id_resume = $id_resume")->result();
     ...
     function getberkas_resume_berkas($id_resume)
     {
          $sql = $this->db->query("SELECT * from t_upload_resume 
                                        join t_resume on t_resume.id_resume = t_upload_resume.id_resume
                                        where t_upload_resume.id_resume = $id_resume")->result();
	 ...
	 function getnonlit_id ($id){
		 $sql = $this->db->query("SELECT * from t_nonlit where id_nonlit = $id
                                        ORDER BY tgl_nonlit DESC")->row();
          return $sql;  
		 
	 }
	 
     function get_nonlitrow($id)
     {
          $sql = $this->db->query("SELECT * from t_nonlit left join t_resume on t_nonlit.id_nonlit = t_resume.id_nonlit
                                        where t_nonlit.id_nonlit = $id
                                        ORDER BY tgl_resume DESC")->row();
          return $sql;     
     }

     function get_data_perkara_to()
     {
          $sql = $this->db->query("SELECT perkara_id,
                                        ...
                                        WHEN perkaradet_tingkat = 'Banding'
                                             AND perkaradet_status = 'menang'
                                             AND perkaradet_aktiv = 'y' THEN
                                                  'Proses Kontra Kasasi'

                                                  
                                        WHEN perkaradet_tingkat = 'Banding'
                                             AND perkaradet_status = 'kalah'
                                             AND perkaradet_aktiv = 'y' THEN
                                                  'Proses Penyataan Kasasi'
                                        ...
                                        WHEN perkaradet_tingkat = 'Kasasi'
                                             AND perkaradet_status = 'kalah'
                                             AND perkaradet_aktiv = 'y' THEN
                                                  'Proses Pernyataan PK'

                                        WHEN perkaradet_tingkat = 'Tingkat 1'
                                        AND perkaradet_status = 'menang'
                                             AND perkaradet_aktiv = 'y' THEN
                                                  'Proses Kontra Banding'

                                                  
                                        WHEN perkaradet_tingkat = 'Tingkat 1'
                                        AND perkaradet_status = 'kalah'
                                             AND perkaradet_aktiv = 'y' THEN
                                                  'Proses Pernyataan Banding'
...
// exit();
     ...
     function insertdata_det($data,$id){
       ...
       {
           $exe = $this->db->where('id_nonlit', $id);
           $exe = $this->db->update('t_nonlit', $data);
           if ($exe) {
               return '1';
           } else {
               return '0';
       ...
       { 
           $exe = $this->db->where('id_resume', $id);
           $exe = $this->db->update('t_resume', $data);
           if ($exe) {
               return $id;
           } else {
               return '0';
